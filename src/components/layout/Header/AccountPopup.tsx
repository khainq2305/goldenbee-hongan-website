"use client";

import React from "react";
import Image from "next/image";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";

const AccountPopup = ({ onClose }: { onClose: () => void }) => {
  const objectFitStyle: React.CSSProperties["objectFit"] = "contain";

  return (
    <div
      className="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] px-4"
      onClick={onClose}
    >
      <div
        className="bg-white rounded-xl shadow-lg w-full max-w-[800px] flex flex-col md:flex-row  relative animate-scaleIn"
        style={{ minHeight: "480px" }}
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          aria-label="Đóng"
          className="
            absolute
            -top-[21px]
            -right-[21px]
            w-[42px]
            h-[42px]
            bg-white
            rounded-full
            shadow
            flex
            items-center
            justify-center
            transition-colors
            hover:bg-gray-100
            z-10
          "
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={2}
            stroke="currentColor"
            className="w-[24px] h-[24px] text-gray-400"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M6 18L18 6M6 6l12 12"
            />
          </svg>
        </button>

        <div className="w-full md:w-1/2 px-8 py-10 flex flex-col">
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Xin chào,</h2>
          <p className="text-gray-600 mb-6 text-base">
            Đăng nhập hoặc Tạo tài khoản
          </p>

          <div className="mb-8">
            <Input
              type="tel"
              placeholder="Số điện thoại"
              className="
                w-full text-lg py-2 px-0
                bg-transparent rounded-none border-0 border-b border-gray-200
                text-gray-900 placeholder-gray-300 placeholder:text-lg
                focus:outline-none focus:ring-0 focus:border-b-gray-200
                shadow-none
              "
            />
          </div>

          <Button
            onClick={() => {}}
            variant="primary"
            size="md"
            className="mb-6"
          >
            Tiếp tục
          </Button>

          <div className="relative mb-6">
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-gray-200"></div>
            </div>
            <div className="relative flex justify-center text-xs">
              <span className="px-2 bg-white text-gray-400">
                Hoặc tiếp tục bằng
              </span>
            </div>
          </div>

          <div className="flex justify-center items-center space-x-4 mb-8">
            <button
              type="button"
              aria-label="Đăng nhập bằng Facebook"
              className="flex items-center justify-center w-12 h-12 border border-gray-200 rounded-full transition-shadow hover:bg-gray-50"
            >
              <svg
                className="w-7 h-7"
                fill="#1877F2"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M22.676 0H1.324C.593 0 0 .593 0 1.324v21.352C0 23.407.593 24 1.324 24h11.494v-9.294H9.692v-3.622h3.126V8.413c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12V24h6.116c.73 0 1.323-.593 1.323-1.324V1.324C24 .593 23.407 0 22.676 0z" />
              </svg>
            </button>

            <button
              type="button"
              aria-label="Đăng nhập bằng Google"
              className="flex items-center justify-center w-12 h-12 border border-gray-200 rounded-full transition-shadow hover:bg-gray-50"
            >
              <svg
                className="w-6 h-6"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  fill="#FBBC05"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
              </svg>
            </button>
          </div>

          <p className="text-xs text-gray-500 text-center leading-relaxed">
            Bằng việc tiếp tục, bạn đã đọc và đồng ý với
            <a
              href="/dieu-khoan-su-dung"
              className="text-[var(--color-primary)] hover:underline mx-1"
            >
              điều khoản sử dụng
            </a>
            và
            <a
              href="/chinh-sach-bao-mat"
              className="text-[var(--color-primary)] hover:underline ml-1"
            >
              Chính sách bảo mật
            </a>
            của Hồng Ân.
          </p>
        </div>

        <div className="hidden md:flex md:w-1/2 bg-pink-50 p-8 flex-col items-center justify-center text-center rounded-tr-xl rounded-br-xl">
          <Image
            src="/images/header/accountPopup.png"
            alt="Mua sắm tại Hồng Ân"
            width={260}
            height={260}
            style={{ objectFit: objectFitStyle }}
            priority
          />
          <h3 className="text-2xl font-semibold text-[var(--color-primary)] mt-4">
            Mua sắm tại Hồng Ân
          </h3>
          <p className="text-[var(--color-primary)] font-normal mt-1">
            Siêu ưu đãi mỗi ngày
          </p>
        </div>
      </div>
    </div>
  );
};

export default AccountPopup;
